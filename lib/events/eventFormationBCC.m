function varargout = eventFormationBCC(fcn, varargin)
...
%% init
    function [elements, msg] = eventFormationBCC_init(SDS, event)
...
%% get
    function X = eventFormationBCC_get()
	X = P;
    end

	
%% restore
    function [elements,msg] = eventFormationBCC_restore(SDS,X)
...
%% eventTimes
    function eventTimes = eventFormationBCC_eventTimes(~, P0)
        ...
        t0 = P.time0(P0.subset);    % age limit
        ...
        elseif t2 > 0
            % Phase 2 (during BCC)
            t = P.expLinear(alpha2, beta2, t0, Pt);
            
            idx3 = t > t2;         % event times during phase 3
            if any(idx3)
                T2 = P.intExpLinear(alpha2(idx3), beta2(idx3), min(t2, t0(idx3)), t2);
                t(idx3) = P.expLinear(alpha3(idx3), P.beta, max(t2, t0(idx3)), Pt(idx3) - T2);
            end
            
        else
            % Phase 3 (after BCC)
            t = P.expLinear(alpha3, P.beta, t0, Pt);
        end
        
        P.eventTimes(P0.subset) = t;
        eventTimes = P.eventTimes;
    end


%% advance
    function eventFormationBCC_advance(P0)
...
%% fire
    function [SDS, P0] = eventFormationBCC_fire(SDS, P0)
...
%% enable
    function eventFormationBCC_enable(P0)
...
%% block
    function eventFormationBCC_block(P0)
...
%% name
function name = eventFormationBCC_name
...
%% properties
function [props, msg] = eventFormationBCC_properties
...
%% repmat
function B = eventFormationBCC_repmat(A, M, N)
...
%% meshgrid
function [xx, yy] = eventFormationBCC_meshgrid(x, y)
% included for performance ==> might move to spTools
