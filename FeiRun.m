function FeiRun(run, population, repeat, folder)
if ~isdeployed
    addpath( [fileparts(which(mfilename)) '/lib'] );
else
    run = str2num(run);
    population = str2num(population);
end
warning off
...
filename =  sprintf('run_%s_parameter_%f_%f_%f_%f_%f_%f_%f_%f_%f_%f_%f_%f_%f_%f_%f_%f.csv',repeat,parameter);
csvwrite(sprintf('%s/MonteCarloResults/%s',folder,filename), [parameter results' number_of_infections]);
filename =  sprintf('run_%s_parameter_%f_%f_%f_%f_%f_%f_%f_%f_%f_%f_%f_%f_%f_%f_%f_%f.mat',repeat,parameter);
save(sprintf('%s/SDSResults/%s',folder,filename),'SDS') %the function variable needs to be a string!
